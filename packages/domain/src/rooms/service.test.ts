...
class MemoryRoomRepository implements RoomRepository {
  rooms = new Map<string, RoomSummaryRecord & { inviteHash: string }>();
  ...
  async createRoom(input: Parameters<RoomRepository["createRoom"]>[0]) {
    this.rooms.set(input.id, { id: input.id, name: input.name, status: "ACTIVE", memberCount: 1, role: "OWNER", inviteHash: input.inviteTokenHash });
  ...
  async joinByInvite(input: Parameters<RoomRepository["joinByInvite"]>[0]) {
    const room = [...this.rooms.values()].find((candidate) => candidate.inviteHash === input.inviteTokenHash && candidate.status === "ACTIVE");
    if (!room) return null;
...
describe("RoomService", () => {
  it("atomically creates a private room, owner membership and isolated initial balance", async () => {
    ...
    const created = await service.create({ userId: "alice", name: "决赛之夜", rulesAccepted: true });
  ...
  it("requires current room rules before creating or joining", async () => {
    const { service } = setup();
    await expect(service.create({ userId: "alice", name: "决赛之夜", rulesAccepted: false })).rejects.toMatchObject({ code: "ROOM_RULES_REQUIRED" });
  });
  ...
  it("concurrent repeated joins create one membership, one account and one grant", async () => {
    const { repository, service } = setup();
    const room = await service.create({ userId: "alice", name: "决赛之夜", rulesAccepted: true });
