...
export class DrizzleRoomRepository implements RoomRepository {
  ...
  async createRoom(input: Parameters<RoomRepository["createRoom"]>[0]) {
    await this.db.transaction(async (tx) => {
      await tx.insert(rooms).values({ id: input.id, name: input.name, visibility: input.visibility, inviteTokenHash: input.inviteTokenHash, createdBy: input.ownerId, createdAt: input.now, updatedAt: input.now });
