...
export function validatePrewarmCompetitions(targets: CompetitionTarget[]) {
  ...
  if (targets.length > 29) throw new PrewarmConfigurationError("Supplier prewarm supports at most 29 competitions per run because status calibration uses one of the 30 daily static requests.");
}
...
export async function runSupplierPrewarm(dependencies: Dependencies) {
  ...
  try {
    ...
    for (const [index, fixture] of targets.entries()) {
      const result = await dependencies.supplier.run({ type: "PREMATCH_ODDS", attempt: 0, payload: { fixtureId: fixture.supplierFixtureId, matchId: fixture.id, bookmakerId: dependencies.bookmakerId } });
      if (result.outcome === "SUCCESS") oddsSynced += result.synced; else oddsSkipped += 1;
      if (result.outcome === "DEFERRED") { oddsSkipped += targets.length - index - 1; break; }
    }
