...
export function RoomTicketHistoryView({ roomId, isOwner, initialPostMatchTicketVisible }: { roomId: string; isOwner: boolean; initialPostMatchTicketVisible: boolean }) {
  ...
  return <section aria-labelledby="room-ticket-history-title" className="surface p-5">
    ...
    {isOwner && <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-xl border border-[var(--line)] p-4"><input type="checkbox" checked={postMatchVisible} disabled={saving} onChange={(event) => void updatePostMatchVisibility(event.target.checked)} className="mt-1 size-5 accent-[var(--field)]"/><span><strong className="block">开赛后公开完整记录</strong><span className="mt-1 block text-xs leading-5 text-[var(--muted)]">关闭后，其他成员只能看到已提交状态；每个人仍能查看自己的完整记录。</span></span></label>}
...
function Fact({ label, value }: { label: string; value: string }) { return <div><dt className="text-[10px] text-[var(--muted)]">{label}</dt><dd className="mt-1 font-bold tabular-nums">{value}</dd></div>; }
