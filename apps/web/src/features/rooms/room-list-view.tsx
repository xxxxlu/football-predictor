...
export function RoomListView() {
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,.72fr)]">
    <section aria-labelledby="my-rooms-title">
      <div className="mb-5 flex items-end justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[var(--field)]">私人房间</p><h2 id="my-rooms-title" className="kinetic mt-1 text-3xl">我的球队墙</h2></div><span className="league-pill shrink-0">{rooms.length} 个</span></div>
      {loading ? <DataStatePanel state="loading" title="正在加载房间" description=""/> : loadError ? <DataStatePanel state="error" title="房间加载失败" description={loadError} action={<button type="button" onClick={() => window.location.reload()} className="inline-flex min-h-11 items-center justify-center rounded-full border-2 border-[var(--ink)] px-5 font-bold transition hover:bg-[var(--ink)] hover:text-white">重新加载</button>}/> : rooms.length ? <ul className="grid gap-4 sm:grid-cols-2">{rooms.map((room) => <li key={room.id}><Link href={`/rooms/${encodeURIComponent(room.id)}`} className="group surface relative flex min-h-36 flex-col justify-between overflow-hidden rounded-xl p-5 no-underline transition duration-200 hover:-translate-y-1 hover:border-[var(--field)] hover:shadow-[0_16px_40px_rgb(15_80_57/16%)]"><div className="flex items-start justify-between gap-3"><TeamCrest name={room.name} className="size-12 text-base"/><span className="league-pill shrink-0">{room.role === "room_owner" ? "房主" : "成员"}</span></div><div className="mt-4"><strong className="block text-lg font-black leading-tight">{room.name}</strong><span className="mt-1 block text-xs text-[var(--muted)]">{room.memberCount === undefined ? "私人房间 · 独立积分" : `${room.memberCount} 人 · 独立 10,000 积分`}</span></div><span aria-hidden="true" className="link-arrow pointer-events-none absolute bottom-5 right-5 text-xl text-[var(--field)] opacity-0 transition group-hover:opacity-100">→</span></Link></li>)}</ul> : <DataStatePanel state="empty" title="还没有房间" description="创建一个私人房间，或打开朋友发来的邀请链接。"/>}
    </section>

    <aside className="surface h-fit rounded-xl p-5 sm:p-6" aria-labelledby="create-room-title">
      <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[var(--field)]">开始比赛日</p><h2 id="create-room-title" className="kinetic mt-1 text-2xl">创建私人房间</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">你会成为房主，并获得一条只应发送给朋友的邀请链接。</p>
      {createError && <div className="mt-4"><StatusMessage tone="error" title="未能创建">{createError}</StatusMessage></div>}
      {created && <div className="mt-4"><StatusMessage tone="success" title="房间已创建">邀请链接只展示在当前结果中；离开后可在房间内重置生成新链接。</StatusMessage><label className="mt-4 block text-xs font-bold" htmlFor={`${nameId}-invite`}>邀请链接</label><input id={`${nameId}-invite`} readOnly value={inviteUrl} className="mt-2 min-h-11 w-full rounded-lg border border-[var(--line)] bg-white px-3 text-sm"/><div className="mt-3 grid gap-2 sm:grid-cols-2"><button type="button" onClick={async () => { try { await copyInvite(inviteUrl); setCopied(true); } catch { setCreateError("浏览器无法自动复制，请手动选择上方链接。"); } }} className="min-h-11 rounded-full border-2 border-[var(--ink)] px-3 font-bold transition hover:bg-[var(--ink)] hover:text-white">{copied ? "已复制" : "复制邀请"}</button><button type="button" onClick={() => router.push(`/rooms/${encodeURIComponent(created.id)}`)} className="min-h-11 rounded-full bg-[var(--field)] px-3 font-bold text-white transition hover:brightness-95">进入房间</button></div></div>}
      <form onSubmit={createRoom} className="mt-5 space-y-4">
        <div><label htmlFor={nameId} className="mb-2 block text-sm font-bold">房间名称</label><input id={nameId} name="name" required minLength={2} maxLength={80} placeholder="例如：周末看球局" className="min-h-12 w-full rounded-lg border border-[var(--line)] bg-white px-3"/></div>
        ...
        <button disabled={creating} className="inline-flex min-h-12 w-full items-center justify-center rounded-full bg-[var(--ink)] px-4 font-bold text-white transition hover:bg-[var(--field)] disabled:opacity-55">{creating ? "正在创建…" : "创建房间并生成邀请"}</button>
      </form>
    </aside>
  </div>;
}
