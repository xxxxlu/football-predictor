...
export function RoomListView() {
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,.72fr)]">
    ...
    <aside className="surface h-fit rounded-xl p-5 sm:p-6" aria-labelledby="create-room-title">
      <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[var(--field)]">开始你的赛事</p><h2 id="create-room-title" className="kinetic mt-1 text-2xl">创建竞猜群组</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">公开群组会出现在大厅；私人群组仅通过邀请链接加入。群组长期保留，但同一时间只进行一轮赛事竞猜。</p>
      <section className="mt-4 rounded-xl border border-[var(--line)] bg-[var(--wash-brand-soft)] p-4" aria-labelledby="round-lifecycle-title">
        ...
        <ol className="mt-2 grid gap-2 text-xs leading-5 text-[var(--muted)]">
          ...
          <li><b className="text-[var(--ink)]">4. 下一轮：</b>本轮全部结算后，群组自动回到可开启下一轮状态；个人结果可在「我的战绩」查询。</li>
        </ol>
      </section>
      ...
      <form onSubmit={createRoom} className="mt-5 space-y-4">
        <fieldset><legend className="mb-2 text-sm font-bold">竞猜赛事</legend><div className="grid grid-cols-2 gap-2"><label className={`cursor-pointer rounded-xl border p-3 text-sm ${sportChoice === "FOOTBALL" ? "border-[var(--field)] bg-[var(--wash-brand-soft)]" : "border-[var(--line)]"}`}><input name="sport" type="radio" value="FOOTBALL" checked={sportChoice === "FOOTBALL"} onChange={() => setSportChoice("FOOTBALL")} className="mr-2 accent-[var(--field)]"/><strong>足球</strong><span className="mt-1 block text-xs text-[var(--muted)]">联赛与杯赛比赛</span></label><label className={`cursor-pointer rounded-xl border p-3 text-sm ${sportChoice === "FORMULA_1" ? "border-[var(--field)] bg-[var(--wash-brand-soft)]" : "border-[var(--line)]"}`}><input name="sport" type="radio" value="FORMULA_1" checked={sportChoice === "FORMULA_1"} onChange={() => setSportChoice("FORMULA_1")} className="mr-2 accent-[var(--field)]"/><strong>F1 赛车</strong><span className="mt-1 block text-xs text-[var(--muted)]">Race Weekend 场次</span></label></div><p className="mt-2 text-xs leading-5 text-[var(--muted)]">每个房间只围绕一种赛事竞猜，创建后不可更改。</p></fieldset>
        ...
        {/* F1 玩法不分档（正赛=冠军+领奖台之争，排位=杆位），档位只对足球有意义。 */}
        ...
        <label htmlFor={rulesId} className="flex cursor-pointer items-start gap-3 text-sm leading-6"><input id={rulesId} name="rulesAccepted" type="checkbox" required className="mt-1 size-5 shrink-0 accent-[var(--field)]"/><span>我确认每轮赛事会自动封盘，全部结算后才能开启下一轮；个人战绩仍可查询。积分不可购买、转让、提现或兑换。 <Link href="/terms" className="font-bold underline">查看规则</Link></span></label>
        <button disabled={creating} className="inline-flex min-h-12 w-full items-center justify-center rounded-full bg-[var(--ink)] px-4 font-bold text-white transition hover:bg-[var(--field)] disabled:opacity-55">{creating ? "正在创建…" : "创建竞猜群组"}</button>
      </form>
    </aside>
  </div>;
}
