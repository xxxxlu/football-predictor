...
export function RoomDetailView({ roomId }: { roomId: string }) {
  ...
  return <div className="space-y-6">
    {detail.status === "CLOSED" && <StatusMessage tone="success" title="本轮房间已结算并结束">
      所有已提交竞猜均已结算；房间已从「我的房间」和公开大厅移除，不能再加入或提交新竞猜。个人结果、赔率与盈亏请到「我的战绩」查询。
    </StatusMessage>}
    ...
    <section className="surface border-l-4 border-l-[var(--field)] p-5" aria-labelledby="round-explainer-title">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[var(--field)]">ROUND LIFECYCLE</p><h2 id="round-explainer-title" className="display mt-1 text-2xl font-bold">这是一次性竞猜轮次</h2></div><span className="league-pill">{detail.status === "ACTIVE" ? "正在接受竞猜" : detail.status === "RESTRICTED" ? "等待官方结果" : "本轮已清除"}</span></div>
      <div className="mt-4 grid gap-3 text-sm leading-6 text-[var(--muted)] sm:grid-cols-2">
        ...
        <p><b className="text-[var(--ink)]">结算后：</b>当本轮全部已提交竞猜结算完成，房间会从列表清除；个人战绩保留在「我的战绩」。</p>
      </div>
    </section>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
      ...
      {detail.status !== "ACTIVE" ? <aside className="surface h-fit p-5"><h2 className="display text-xl font-bold">本轮已结束</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">这轮不会自动接入随机赛事，也不会继续接受竞猜。房间入口会清除；你的个人战绩可在「我的战绩」查询。</p></aside> : detail.visibility === "PUBLIC" ? <aside className="surface h-fit p-5"><h2 className="display text-xl font-bold">公开房间</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">此房间会显示在公开大厅，任何已登录用户确认本轮规则后都可以加入，无需邀请链接。</p></aside> : detail.isOwner ? <aside className="surface h-fit p-5" aria-labelledby="invite-title"><h2 id="invite-title" className="display text-xl font-bold">邀请朋友</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">请先确认参与者理解：赛事开始会自动封盘，全部结算后本轮会清除。生成新链接会让旧链接立即失效，不影响现有成员和积分。</p>{inviteError && <div className="mt-4"><StatusMessage tone="error" title="邀请操作失败">{inviteError}</StatusMessage></div>}{inviteToken ? <div className="mt-4"><StatusMessage tone="success" title="新邀请已生成">请只发送给你认识的人。</StatusMessage><label htmlFor="room-invite-url" className="mt-4 block text-xs font-bold">邀请链接</label><input id="room-invite-url" readOnly value={inviteUrl} className="mt-2 min-h-11 w-full rounded-lg border border-[var(--line)] bg-white px-3 text-sm"/><button type="button" onClick={async () => { try { await navigator.clipboard.writeText(inviteUrl); setCopied(true); } catch { setInviteError("浏览器无法自动复制，请手动选择上方链接。"); } }} className="mt-3 min-h-11 w-full rounded-full border-2 border-[var(--ink)] px-3 font-bold transition hover:bg-[var(--ink)] hover:text-white">{copied ? "已复制" : "复制邀请链接"}</button></div> : <button type="button" onClick={resetInvite} disabled={resetting} className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-full bg-[var(--field)] px-3 font-bold text-white transition hover:brightness-95 disabled:opacity-55">{resetting ? "正在生成…" : "生成新的邀请链接"}</button>}</aside> : <aside className="surface h-fit p-5"><h2 className="display text-xl font-bold">成员权限</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">赛事开始后自动封盘。你可以查看自己的个人战绩；只有房主可以重置邀请。</p></aside>}
    </div>
    ...
    <section className="surface p-5" aria-labelledby="room-rules-title">
      ...
      <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted)]">
        <li>一个房间是一轮独立竞猜：全部已提交竞猜结算后自动结束并从常规列表移除，不能继续竞猜或加入；个人战绩不随房间入口清除。</li>
