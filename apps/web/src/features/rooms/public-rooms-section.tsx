...
export function PublicRoomsSection({ headingId = "public-rooms-title" }: { headingId?: string }) {
  ...
  return <section aria-labelledby={headingId}>
    <div className="mb-5 flex items-end justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[var(--field)]">{t("rooms.public.eyebrow")}</p><h2 id={headingId} className="kinetic mt-1 text-3xl">{t("rooms.public.title")}</h2></div><span className="league-pill shrink-0">{publicRooms.length} {t("rooms.public.countUnit")}</span></div>
