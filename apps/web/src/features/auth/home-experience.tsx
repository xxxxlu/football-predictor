...
function GuestHome() {
  return <main id="main-content"><section className="mx-auto max-w-7xl px-4 py-12 md:px-8 md:py-20"><p className="eyebrow">朋友间的比赛日记录</p><h1 className="display mt-4 max-w-4xl text-[clamp(2.65rem,7vw,5.6rem)] font-bold leading-[.93]">判断留在账本，<br/><em className="font-normal text-[var(--field)]">输赢交给球场。</em></h1><p className="mt-7 max-w-xl text-lg leading-8 text-[var(--muted)]">创建私人房间，用每房间独立的 10,000 虚拟积分表达判断强度。真实赛程、平台虚拟积分倍率、自动封盘与结算，每一笔变化都有记录。</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/register" className="bg-[var(--field)] px-6 py-3.5 text-center font-bold text-white no-underline hover:bg-[var(--field-dark)]">免费创建账户</Link><Link href="/login" className="border border-[var(--ink)] px-6 py-3.5 text-center font-bold no-underline hover:bg-white/60">我已有账户</Link></div><p className="mt-4 text-xs text-[var(--muted)]">无需手机号或邮箱 · 不涉及现金 · 18+</p></section><section className="border-y rule bg-[var(--ink)] text-white"><div className="mx-auto grid max-w-7xl gap-px bg-white/20 md:grid-cols-3"><Feature number="01" title="每房独立" text="加入每个房间都从 10,000 分开始，互不影响。"/><Feature number="02" title="服务端封盘" text="提交前复核积分倍率和开球状态，失败不扣分。"/><Feature number="03" title="可审计结算" text="冻结、结算、更正与冲正形成完整时间线。"/></div></section></main>;
}
