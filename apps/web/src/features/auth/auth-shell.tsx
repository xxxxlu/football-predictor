...
import { sharedAuthCopy } from "@/lib/i18n/messages";
...
export function AuthShell({ eyebrow, title, description, children, footer }: { eyebrow: string; title: string; description: string; children: React.ReactNode; footer: React.ReactNode }) {
  ...
  const localizedCopy = locale === "en" ? sharedAuthCopy[title] : undefined;
  return <main id="main-content" className="grid min-h-screen lg:grid-cols-[minmax(20rem,.72fr)_1.28fr]">
    <aside className="night line-art relative hidden overflow-hidden border-r border-[var(--night-line)] p-12 text-white lg:flex lg:flex-col lg:justify-between">
      ...
      <blockquote className="relative">
        ...
        <p className="mt-5 max-w-xs text-sm leading-6 text-white/60">{locale === "en" ? "Every call deserves a record. Predict football and F1 with friends using virtual points." : "每一次判断，都应该留下记录。足球、F1，和朋友一起预测，用虚拟积分留下每一次判断。"}</p>
        <PulseLine state="upcoming" className="mt-8 h-7 w-full max-w-xs" />
      </blockquote>
      <p className="relative text-xs leading-5 text-white/55">{locale === "en" ? "No deposits · No withdrawals · No exchanges · 18+" : "无充值 · 无提现 · 无兑换 · 仅限 18+"}</p>
    </aside>
    <section className="flex min-h-screen flex-col">
      <header className="flex items-center justify-between border-b rule px-4 py-4 sm:px-8">
        <div className="lg:hidden"><BrandMark /></div>
        <div className="ml-auto flex items-center gap-4"><LanguageSwitcher compact /><Link href="/" className="text-sm font-bold underline-offset-4 hover:underline">{t("auth.backHome")}</Link></div>
      </header>
      <div className="flex flex-1 items-center justify-center px-4 py-10 sm:px-8">
        <div className="w-full max-w-md">
          <p className="eyebrow">{localizedCopy?.eyebrow ?? eyebrow}</p>
          <h1 className="display mt-3 text-4xl font-bold">{localizedCopy?.title ?? title}</h1>
          <p className="mt-3 leading-7 text-[var(--muted)]">{localizedCopy?.description ?? description}</p>
