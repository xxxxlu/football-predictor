...
export function ChangePasswordForm() {
  ...
  return <form onSubmit={submit} className="space-y-5">{error && <StatusMessage tone="error" title="操作失败">{error}</StatusMessage>}<label className="block text-sm font-bold">当前初始密码<input name="currentPassword" type="password" autoComplete="current-password" required minLength={12} maxLength={128} className="mt-2 min-h-12 w-full rounded-lg border border-[var(--line)] px-3"/></label><label className="block text-sm font-bold">新密码<input name="newPassword" type="password" autoComplete="new-password" required minLength={12} maxLength={128} className="mt-2 min-h-12 w-full rounded-lg border border-[var(--line)] px-3"/></label><button disabled={pending || loggingOut} className="inline-flex min-h-12 w-full items-center justify-center rounded-full bg-[var(--field)] px-5 font-bold text-white transition hover:brightness-95 disabled:opacity-60">{pending ? "正在更新…" : "更新密码并继续"}</button><button type="button" onClick={logout} disabled={pending || loggingOut} className="inline-flex min-h-12 w-full items-center justify-center rounded-full border-2 border-[var(--ink)] px-5 font-bold transition hover:bg-[var(--ink)] hover:text-white disabled:opacity-60">{loggingOut ? "正在退出…" : "退出当前账户"}</button><p className="text-xs leading-5 text-[var(--muted)]">暂不修改时可以安全退出；重新登录后仍需完成初始密码更新。</p></form>;
}
