...
import { createClubChannelRepository, createIdentityDatabase, type ClubChannelRepository } from "@pulse/db";
...
export function lobbyHandlers() {
  if (!globalThis.__pulseClubChannelRepository) {
    ...
    globalThis.__pulseClubChannelRepository = createClubChannelRepository(createIdentityDatabase(config.databaseUrl).sql);
  }
