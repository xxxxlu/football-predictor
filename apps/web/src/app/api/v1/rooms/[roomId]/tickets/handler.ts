...
function ticketError(error: TicketSubmissionError) {
  const status = error.code === "DATA_UNAVAILABLE" ? 503
    : error.code === "MARKET_CLOSED" || error.code === "ODDS_CHANGED" || error.code === "SCORE_TICKET_EXISTS" || error.code === "MARKET_TICKET_EXISTS" || error.code === "ROUND_IN_PROGRESS" ? 409
  ...
  const messages: Record<TicketSubmissionError["code"], string> = {
    ...
    ROUND_IN_PROGRESS: "This group is settling another event round. Start the next round after every current ticket settles.",
  };
