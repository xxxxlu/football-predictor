...
export const DELETE = (request: Request, context: { params: Promise<{ roomId: string; messageId: string }> }) => context.params.then(({ roomId, messageId }) => chatHandlers().unpin(request, roomId, messageId));
