...
import { createRoomChatRepository, getSharedIdentityDatabase, type RoomChatRepository } from "@pulse/db";
...
export function chatHandlers() {
  if (!globalThis.__pulseRoomChatRepository) {
    ...
    globalThis.__pulseRoomChatRepository = createRoomChatRepository(getSharedIdentityDatabase(config.databaseUrl).sql);
  }
