...
- FR73（Phase 2）: 用户创建房间时可选择档位 STANDARD 或 ADVANCED；档位创建后不可变，既有房间默认 STANDARD。
- FR74（Phase 2）: 只有 ADVANCED 房间开放正确比分盘；服务端强制门禁，STANDARD 房间提交比分预测必须被拒绝，前端隐藏不作为权限边界。
- FR75（Phase 2）: ADVANCED 房间用户可从平台比分候选集（16 个明列比分加 OTHER 兜底）选择一个比分投入积分，提交前可查看该比分倍率与预计返还。
- FR76（Phase 2）: 系统按平台固定虚拟倍率表提供比分赔率并标注为平台虚拟积分规则；比分盘不依赖外部赔率源。
- FR77（Phase 2）: 同一用户在同一房间对同一比赛只能持有一张未结算比分票；重复提交被服务端拒绝且不冻结积分。
- FR78（Phase 2）: 系统按最终比分完全相等判定比分盘，OTHER 在最终比分不属于明列集合时为赢；取消退款、幂等、账本追加与赛果更正规则与 1X2 一致。
- FR79（Phase 2）: 1X2 与正确比分盘在同一比赛并存；比分盘为加法能力，赔率版本与 ODDS_CHANGED 重新确认、服务端封盘、原子冻结等既有约束同样适用。
...
- FR73: Epic 8 — 扩展公开竞技与高级预测
- FR74: Epic 8 — 扩展公开竞技与高级预测
- FR75: Epic 8 — 扩展公开竞技与高级预测
- FR76: Epic 8 — 扩展公开竞技与高级预测
- FR77: Epic 8 — 扩展公开竞技与高级预测
- FR78: Epic 8 — 扩展公开竞技与高级预测
- FR79: Epic 8 — 扩展公开竞技与高级预测
...
### Epic 8: 扩展公开竞技与高级预测
正式产品用户可使用补分流程、高级盘口（含正确比分）、串关、公开大厅、公开排行、长期战绩与分享卡。
**Phase:** Phase 2
**FRs covered:** FR43, FR44, FR45, FR52, FR53, FR61, FR62, FR63, FR73, FR74, FR75, FR76, FR77, FR78, FR79
...
### Story 8.5: 在高级房间买正确比分

As a ADVANCED 房间成员,
I want 对开放比赛买一个正确比分,
So that 在 1X2 之外获得更高倍率的平台虚拟玩法.

**Requirements:** FR73, FR74, FR75, FR76, FR77, FR78, FR79

**Acceptance Criteria:**

**Given** 用户创建或进入 ADVANCED 房间的开放比赛
**When** 从平台比分候选集（16 个明列比分加 OTHER）选择一个并投入不超过 20,000 且不超过可用余额的积分
**Then** 服务端复核封盘、比分盘赔率版本与可用余额后原子冻结积分并写入冻结流水
**And** 提交前展示该比分平台虚拟倍率与预计返还，并标注为平台虚拟积分规则

**Given** STANDARD 房间用户或对同一比赛已有未结算比分票的用户提交比分预测
**When** 请求到达服务端
**Then** 分别以 ADVANCED_ROOM_REQUIRED 或 SCORE_TICKET_EXISTS 拒绝且不冻结积分
**And** 既有 1X2 预测、赔率、结算与展示行为保持不变

**Given** 供应商确认最终比分
**When** 结算比分票
**Then** 所选比分与最终比分完全相等为赢、OTHER 在最终比分不属于明列集合时为赢、否则为输
**And** 取消退款、幂等、账本追加与赛果更正冲正重结规则与 1X2 完全一致（比分盘无走盘/PUSH）
